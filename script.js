...
let chatHistory = [
  {
    role: "system",
    content:
      "You are a helpful beauty advisor. Only answer questions about the generated routine, skincare, haircare, makeup, fragrance, or other beauty-related topics. If a question is off-topic, politely guide the user back to beauty and routine advice. Always use the full conversation history for context.",
  },
];
...
generateBtn.addEventListener("click", async () => {
  ...
  try {
    const response = await fetch(
      "https://loreal-worker.steven87.workers.dev/",
      {
        method: "POST",
        headers: {
    ...
    if (
      ...
      chatWindow.innerHTML +=
        '<div class="placeholder-message">Sorry, I could not generate a routine. Please try again.</div>';
    }
  } catch (error) {
    chatWindow.innerHTML +=
      '<div class="placeholder-message">Error connecting to OpenAI. Please try again later.</div>';
  }
  // Clear input
...
chatForm.addEventListener("submit", async (e) => {
  ...
  chatWindow.innerHTML =
    chatHistory
      ...
      .join("") + '<div class="placeholder-message">Thinking...</div>';
  ...
  try {
    ...
    if (
      ...
      chatWindow.innerHTML +=
        '<div class="placeholder-message">Sorry, I could not answer that. Please try again.</div>';
    }
  } catch (error) {
    chatWindow.innerHTML +=
      '<div class="placeholder-message">Error connecting to OpenAI. Please try again later.</div>';
